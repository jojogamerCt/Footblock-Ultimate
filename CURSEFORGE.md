...
- **Swerve Shots**: Turn your aim across your body while releasing a shot with the World Cup ball to add airborne curve. Curve physics works without displaying shot or pass telemetry in the action bar.
...
- **Score Manager Console**: Open its dedicated GUI to select a team, add or remove a point, set an exact score, reset both teams, or end the score and remove the sidebar from every player's HUD. The exact-score value and caret are centered inside the custom textbox.
